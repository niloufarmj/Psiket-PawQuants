...
public class InGameUIManager : MonoBehaviour
{
    ...
    public void SetCanvas(bool mobile)
    {
        canvasScaler.matchWidthOrHeight = mobile ? 0.3f : 0;
    }
    ...
    public void ShowWinMenu()
    {
        ...
        if (int.Parse(seconds.text) + (int.Parse(minutes.text) * 60) < 15)
            stars = 3;
        else if (int.Parse(seconds.text) + (int.Parse(minutes.text) * 60) < 30)
            stars = 2;
